...
from da3mvs.bench.print_metrics import MetricsPrinter
from da3mvs.utils.parallel_utils import parallel_execution
from da3mvs.bench.registries import MV_REGISTRY
from da3mvs.utils.constants import EVAL_REF_VIEW_STRATEGY
...
class Evaluator:
    ...
    def _compute_pose_with_gt(self, result_path: str, gt_meta: Dict) -> TDict[str, float]:
        ...
        """
        from da3mvs.bench.dataset import _wait_for_file_ready
        from da3mvs.bench.utils import compute_pose
        from da3mvs.utils.geometry import as_homogeneous
...
if __name__ == "__main__":
    ...
    from da3mvs.cfg import load_config
...
Usage:
    python -m da3mvs.bench.evaluator [OPTIONS] [KEY=VALUE ...]
...
Examples:
  # Use default config
    python -m da3mvs.bench.evaluator

  # Override model path
    python -m da3mvs.bench.evaluator model.path=depth-anything/DA3-LARGE
  ...
  # Multi-GPU inference (auto-distributed)
    CUDA_VISIBLE_DEVICES=0,1,2,3 python -m da3mvs.bench.evaluator
  ...
  # Only evaluate (skip inference)
    python -m da3mvs.bench.evaluator eval.eval_only=true

  # Only print saved metrics
    python -m da3mvs.bench.evaluator eval.print_only=true
    ...
    if print_only:
        ...
        if len(gpu_list) > 1 and not is_worker:
            ...
            # Build base command
            base_cmd = [sys.executable, "-m", "da3mvs.bench.evaluator"]
            ...
            from da3mvs.api import DepthAnything3
