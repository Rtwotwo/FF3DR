...
class FF3DR:
    ...
    def __init__(self, args):
        ...
        logger.info("[INFO] Active MatrixCity block: %s", self.active_block_name)
    ...
    def _resolve_single_block_area_path(self, area_path, block_name):
        """Resolve MatrixCity train path to one concrete block directory."""
...
def _build_parser(run_defaults):
    ...
    parser.add_argument(
        "--run_args_yaml",
        type=str,
        default=str(_REPO_ROOT / "configs" / "run_matrixcity_inference.yaml"),
        help="YAML file with default startup arguments",
    )
    parser.add_argument(
        "--area_path",
        type=str,
        default=run_defaults.get("area_path", "./dataset/MatrixCity/small_city/aerial/train"),
        help="Input folder. Supports one block folder with flat images, or a train folder containing multiple blocks",
    )
    parser.add_argument(
        "--block_name",
        type=str,
        default=run_defaults.get("block_name", ""),
        help="When area_path points to a multi-block train folder, choose one block name, e.g. block_1",
    )
    parser.add_argument(
        "--config_path",
        type=str,
