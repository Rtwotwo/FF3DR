...
def _is_triton_bf16_broken():
    try:
        import torch.utils.cpp_extension as cpp_ext
        cuda_version = cpp_ext.CUDA_HOME
        if cuda_version is not None:
            nvcc_path = os.path.join(cuda_version, "bin", "nvcc")
            if os.path.isfile(nvcc_path):
                import subprocess
                result = subprocess.run([nvcc_path, "--version"], capture_output=True, text=True)
                for line in result.stdout.split("\n"):
                    if "release" in line.lower():
                        ver_str = line.split("release")[-1].strip().split(",")[0].strip()
...
class FF3DR:
    def __init__(self, args):
        ...
        if self.device.startswith("cuda") and torch.cuda.is_available():
            if torch.cuda.is_bf16_supported() and not _is_triton_bf16_broken():
        ...
        logger.info("[INFO] Active UrbanScene block: %s", self.active_block_name)
    ...
    def _get_prediction_arrays(self, pred):
        ...
        if extrinsics is not None:
            if extrinsics.ndim == 3 and extrinsics.shape[-1] == 4:
                extrinsics = extrinsics
            elif extrinsics.ndim == 2 and extrinsics.shape[-1] == 4:
                extrinsics = extrinsics[None, ...]
        if intrinsics is not None:
            if intrinsics.ndim == 2 and intrinsics.shape[-1] == 3:
    ...
    def _resolve_anchor_cam_index(self, anchor_cam_index):
        if anchor_cam_index >= 0:
            return min(anchor_cam_index, self.num_cameras - 1)
        return 0
    ...
    def _select_anchor_from_frame_major(self, frame_major_data, overlap_n, from_tail=False):
        if frame_major_data is None or self.num_cameras == 0:
            return None
        anchor_stride = frame_major_data.shape[0] // self.num_cameras
        if from_tail:
            start = anchor_stride - overlap_n
            end = anchor_stride
        else:
            start = 0
            end = overlap_n
        start = max(0, start)
        end = min(anchor_stride, end)
        anchor_data = frame_major_data[start::self.num_cameras]
        return anchor_data
    ...
    def inference_single_chunk(self, range_1, chunk_idx=None, range_2=None, is_loop=False):
        ...
        if self.enable_nvs_rendering and not is_loop and "predictions" in predictions:
            self._run_nvs_rendering(chunk_idx, predictions.pop("predictions"))
    ...
    def _run_nvs_rendering(self, chunk_idx, predictions):
        if not self.enable_nvs_rendering:
            return

        try:
            gs_dir = os.path.join(self.output_path, "gs_output", f"chunk_{chunk_idx}")
            os.makedirs(gs_dir, exist_ok=True)

            logger.info("[NVS] Generating Gaussian Splatting for chunk %d", chunk_idx)

            for pred_idx, prediction in enumerate(predictions):
                if prediction.gaussians is None:
                    logger.warning("[NVS] Prediction %d has no gaussians, skipping", pred_idx)
                    continue

                self._export_to_gs_ply(
                    prediction=prediction,
                    export_dir=gs_dir,
                )

                self._export_to_gs_video(
                    prediction=prediction,
                    export_dir=gs_dir,
                    trj_mode="extend",
                    color_mode="RGB+D",
            ...
            logger.info("[NVS] Saved NVS rendering for chunk %d to %s", chunk_idx, gs_dir)

        except Exception as e:
            logger.warning("[NVS] NVS rendering failed for chunk %d: %s", chunk_idx, str(e))
    ...
    def run(self):
        logger.info("[INFO] Running FF3DR on UrbanScene: %s", self.area_path)
...
def _build_parser(run_defaults):
    ...
    parser.add_argument(
        ...
        help="Input folder for UrbanScene dataset",
    )
    parser.add_argument(
        "--block_name",
        type=str,
        default=run_defaults.get("block_name", ""),
        help="When area_path points to multi-block folder, choose one block name",
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default=run_defaults.get("output_path", "./exp/urbanscene/run_urbanscene"),
    ...
    parser.add_argument(
        ...
        help="Explicit camera folder names to use",
    )
    parser.add_argument(
        "--da3_infer_mode",
        type=str,
        default=run_defaults.get("da3_infer_mode", "global_chunk"),
    ...
    parser.add_argument(
        ...
        help="1: force stable fusion path for DA3 multicam, 0: allow raw framewise stitching",
    )
    parser.add_argument(
        "--da3_group_frames",
        type=int,
    ...
    parser.add_argument(
        "--device",
        type=str,
        default=run_defaults.get("device", "cuda"),
