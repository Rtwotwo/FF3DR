...
class WHUOMVSDataset(Dataset):
    ...
    loading zeras with images, depths, masks and camera parameters
    for training, testing but not for prediction.
    Args:
        dataset_dir: path to the dataset dataset directory
        split: 'train' or 'test'
        num_views: number of views to load for each sample
    ...
    def __init__(self, dataset_dir, 
        ...
        # load all samples containing images, depths, masks and camera parameters
        self.samples = []
        for area in self.areas:
            area_dir =self.dataset_dir / split / area
            self._load_samples_from_area(area_dir)
        logger.info(f'[INFO] {len(self.samples)} samples loaded from {len(self.areas)}.')
    def __len__(self):
        """
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            sample: a dictionary containing images, depths, masks and camera parameters
        """
        ...
        # load multiview sample, default to load 5 views
        if self.num_views > 1:
    ...
    def _load_samples_from_area(self, area_dir):
        """
        load  images, depths, masks and camera parameters from single area
        Args:
    ...
    def _load_mask(self, mask_path):
        ...
        mask = torch.from_numpy(mask).unsqueeze(0) # HWC -> CHW
        return mask
    ...
    def _load_source_views(self, ref_sample, num_src_views):
        """
        load source views for the given reference sample
        Args:
            ref_sample: reference sample containing image, depth, mask and camera parameters
        ...
        # get source view indices (e.g., select neighboring frames)
